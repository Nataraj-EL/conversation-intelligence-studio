export default function Features() {
  const items = [
    ...
    },
    {
      ...
      icon: (
        <svg className="h-6 w-6 text-brand-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
    ...
    },
    {
      ...
      icon: (
        <svg className="h-6 w-6 text-brand-peach" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
  ...
  return (
    <section id="features" className="py-20 bg-bg-base relative">
      {/* Visual background highlight */}
      <div className="absolute bottom-0 right-1/4 w-[350px] h-[350px] rounded-full bg-brand-indigo/10 blur-[100px] pointer-events-none -z-10" />

      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center mb-16">
          <h2 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">
            Everything you need to <span className="bg-gradient-to-r from-brand-blue to-brand-peach bg-clip-text text-transparent">ship reliable AI agents</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {items.map((item, idx) => (
            ...
            >
              {/* Icon container */}
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-bg-surface-hover border border-border-subtle shrink-0 shadow-md">
                {item.icon}
              </div>
